...
| `mas_mission` | ament_python | Mission logic + state machine (BOOT → SURVEY → VERIFY_TAG → CONVERGE → FINISH) |

### Custom Messages (`mas_interfaces/msg`)

| Message | Purpose |
|---|---|
| `PoseBeacon` | Periodic drone pose broadcast for neighbor graph maintenance |
| `SyncHello` / `SyncAck` | Handshake to open a sync window between neighbors |
| `MineDelta` / `MineBelief` | Incremental mine belief updates with seq + TTL |
| `TaskAnnounce` / `TaskClaim` / `TaskResult` | Full auction lifecycle messages |
...
cd ~/ros2_ws/src
ln -s /path/to/mas_coordinator/* .

cd ~/ros2_ws
colcon build --packages-select mas_interfaces mas_task mas_sync mas_mission
